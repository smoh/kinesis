"""Fit hyades DR2 data but use HARPS RVs not DR2"""
import os, sys
import pandas as pd
import numpy as np
...
outfile = "../report/hyades-dr2/fit_harpsrv.pickle"
...
data = (
    df[necessary_columns + ["RV_HARPS_leao", "eRV_HARPS_leao"]].copy().rename(
        columns={'RV_HARPS_leao':'radial_velocity', 'eRV_HARPS_leao':'radial_velocity_error'}
        )
    )

b0 = np.median(data.g.icrs.cartesian.xyz.value, axis=1)
